...
# =====================================================================
#
# 파이토치(Pytorch, 코드에선 torch) = 딥러닝 도구 상자.
# - 사이킷런이 'fit 한 줄에 다 해주는 완성품'이라면, 파이토치는 우리가 01에서 짠 루프를 그대로 쓰되
#   '밟아보기(미분)'와 '한 걸음(갱신)'만 대신해 주는 부품 상자.
#
# 언제 쓰나?
...
#
# 표 데이터 + 선형모델이면 사이킷런이 더 편합니다. 여기선 '같은 답이 나온다'를 확인해서
# "파이토치가 별개의 마법이 아니라 01의 그 루프"라는 걸 아는게 목적
...
# =====================================================================
# 텐서(tensor) = 파이토치의 배열. numpy 배열과 거의 같고, '미분 추적 기능'이 붙어 있는게 차이.
x = df["공기온도"].values
y = df["공정온도"].values
...
# numpy 기본 float64는 15자리 그래서 파일의 결과 끝자리가 01 04 와 0.0002쯤 다를 수 있음.


# =====================================================================
# 2. 자동 미분 — 01 의 '밟아보기'를 파이토치가 대신
# =====================================================================
# 01에선 w를 +-0.001 밀어 손실 차이로 기울기(편미분)를 쟀습니다.
# 손잡이 5개면 5번, 100만 개면 100만번.
#
# 파이토치는 계산 고정을 '기록'해 뒀다가 backward() 한 번에 모든 손잡이의 기울기를 내놓습니다.
# autoagrad(자동 미분) = 계산 과정을 기록해 두고 backward() 한 번으로 모든 손잡이의 편미분을
# 파이토치를 쓰는 첫 번째 이유. 손잡이가 몇백만 개인 신경망에선 밟아보기가 불가능해서 이게 필수.
...
# .item() = 숫자 하나짜리 텐서를 그냥 파이썬 숫자로. 출력하거나 round 할 때 필요

# 정말 맞나? 01 방식(밟아보기)으로 대조 (밟아보기는 아주 작은 차이를 재니, 정밀한 numpy float64로 합니다.)
h = 1e-4
...
# 주의 .grad는 backward 할 때마다 '더해집니다'. 매 걸음 전에 비워야 합니다.


# =====================================================================
# 3. 파이토치 표준 루프 — 부품 세 개 + 네 줄
# =====================================================================
# 부품 (01의 무엇에 해당하는지 보세요):
...
with torch.no_grad():
    ...
    # weight 는 (1, 4) 표 모양. .squeeze(0) = 크기 1인 겉껍질을 벗겨 (4,)로.
...
# =====================================================================
# 03에서 배운 두 가지 변화(sigmoid, 로그손실)를 파이토치는 손실 함수 하나에 담아 둡니다:
# torch.nn.BCEWithLogitsLoss() = "직선값(logit)d을 받아 안에서 sigmoid를 씌우고 로그손실(BCE) 계산"
...
# 04 사이킷런(같은 분할)과 비교: 04는 1등 확률 0.366, 여기선 0.539
# 파이토치엔 규제가 없어 확률이 더 벌어짐
# 하지만 1등(0.539)은 진짜 고장이 아니라 헛경보고, 임계값 0.5에선 여전히 고장을 못 잡습니다.
# 도구가 달라도 결론은 같습니다. 불균형 데이터는 임계값 class_weight로 손 봐야함 (04)


# =====================================================================
# 6. 미니배치 — 데이터가 클 때 조금씩 나눠 걷기 (파이토치를 쓰는 두 번째 이유)
# =====================================================================
# 지금까진 매 걸음 140대 전부로 기울기를 쟀습니다. 데이터가 100만 건이라면 한 걸음이 너무 무겁고 메모리에 안 들어감.
# 그래서 16대씩 뽑아 한 걸음, 다음 16대로 한 걸음... 이렇게 갑니다. DataLoader가 섞고 나눠 줍니다.
# 배치 = 한 걸음에 보는 데이터 묶음. 미니배치 = 그 묶음이 전체의 일부일 때.
# 에폭 = 전체를 한 바퀴 다 보는 것. 배치 16이면 140대는 한 에폭에 9걸음(16x8=128, 남은 12대가 9번째)
# DataLoader = "섞어서 배치 크기만큼 잘라 하니씩 건네주는" 부품.
from torch.utils.data import TensorDataset, DataLoader
# 습관 하나 추가: 정답(y)도 표준화합니다.
...
# 파이토치를 배운 이유?
# 신경망할 때 이 네줄이 그대로 나옴.
# 140대 한꺼번에 500걸음이나, 16대씩 900걸음(100에폭 x9)이나 같은 자리. 데이터가 커도 메모리엔 16대만 올리면 됨
# [메모리?]
# 메모리(RAM) = 작업 책상, 하드디스크 = 책장. 계산은 책상에 펼쳐 놓은 것만 할 수 있습니다.
# 숫자로 확인해 보면:
#     숫자 하나 = 8바이트. 100만행 x 컬럼 100개 = 약 0.8GB. 램이 16GB면 될 것 같습니다.
#     그런데 학습을 시작하면 기울기와 중간 계산값 사본이 몇 개씩 생겨서 3~5배가 됩니다.

# =====================================================================
# 7. 다음 과정 예고 — 층을 쌓으면 신경망 (지금은 한 줄만, 코드 없음)
# =====================================================================
# 지금 model4는 '직선 한 겹'(nn.Linear 하나)
# 8. 정리 — 언제 무엇을 쓰나
# =====================================================================
print("""
[8] 손코드 ↔ 사이킷런 ↔ 파이토치
